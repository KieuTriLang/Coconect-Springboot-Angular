...
public interface MessageRepo extends JpaRepository<Message, Long> {
        ...
        @Query(value = "SELECT DISTINCT * FROM message m WHERE (m.identity_code = :senderCode OR m.receiver_code = :senderCode) AND (m.identity_code = :receiverCode OR m.receiver_code = :receiverCode) AND m.id < :id ORDER BY m.id DESC LIMIT 50", nativeQuery = true)
        ...
        @Query(value = "SELECT DISTINCT * FROM message m WHERE (m.identity_code = :senderCode OR m.receiver_code = :senderCode) AND (m.identity_code = :receiverCode OR m.receiver_code = :receiverCode) ORDER BY m.id DESC LIMIT 50", nativeQuery = true)
